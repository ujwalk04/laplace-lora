...
def main(cfg: DictConfig):
    ...
    checkpoint_freq = 500
    output_file = f"{cfg.paths.output_dir}/results.txt"
    #
    # 3. Do MAP training
    #
    ...
    def fun(model, tokenizer, map_param_path):
        ll_path = f"{cfg.paths.output_dir}/ll_{grad_steps}.pth"
        ...
        kfac_path = f"{cfg.paths.output_dir}/kronecker_factors_{grad_steps}.pth"
        ...
        #
        prior_path = f"{cfg.paths.output_dir}/prior_params{grad_steps}.pth"
        ...
        model1, tokenizer1, gen_cfg = setup_llm(**cfg.llm)
        model1 = peft.PeftModel.from_pretrained(model1, map_param_path, is_trainable=True)
        ...
        with t.no_grad():
            for batch in tqdm(val_loader, disable=not cfg.use_tqdm, file=sys.stdout):
                ...
                batch_inputs = tokenizer1(prompts, **cfg.tokenizer_run_kwargs).to(device)

                # Predict the output logit locations
                jacobian, f_mu = jacobian_mean(
                    model1, batch_inputs, output_callback=output_callback
                )
        ...
        res.append(f"NLL: {loss:.5f}, ACC: {acc:.5f}, ECE: {ece:.5f}")
        with open(output_file, 'a') as f:
            f.write(f"NLL: {loss:.5f}, ACC: {acc:.5f}, ECE: {ece:.5f}")
        output_path = f"{cfg.paths.output_dir}/predicted_logits_{grad_steps}.pth"
        t.save(
    ...
    if not os.path.exists(map_param_path) or cfg.run_every_step:
        ...
        while grad_steps < cfg.train_steps:
            map_param_path = f"{cfg.paths.output_dir}/MAP_params_{grad_steps}.pth"
            ...
            for batch in tqdm(train_loader, disable=not cfg.use_tqdm, file=sys.stdout):
                ...
                opt.step()
                grad_steps += 1
                if grad_steps % checkpoint_freq == 0:
                    model.save_pretrained(map_param_path)
                    fun(model, tokenizer, map_param_path)
